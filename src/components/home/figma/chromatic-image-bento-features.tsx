...
function FeatureColumn({ features }: { features: readonly ChromaticBentoFeature[] }) {
  return (
    <dl className="grid h-full grid-rows-2 divide-y divide-black/10">
      {features.map((feature) => (
        ...
        >
          <dt className="flex items-start justify-between gap-4">
            <span className="text-lg font-medium tracking-tight">{feature.title}</span>
            <span
              className="text-sm tracking-wide text-neutral-400 tabular-nums"
              style={{ fontFamily: "var(--font-mono)" }}
            >
              {feature.number}
            </span>
          </dt>
          <dd className="max-w-[40ch] text-pretty text-base/7 text-neutral-600 sm:text-sm/6">
            {feature.description}
          </dd>
        </div>
      ))}
...
export function ChromaticImageBentoFeatures({
  ...
  return (
    <div className="overflow-hidden rounded-2xl bg-white text-neutral-950 outline-1 -outline-offset-1 outline-black/10">
      <div className="grid lg:grid-cols-[3fr_4fr_3fr]">
        <div className="order-2 border-black/10 lg:order-1 lg:border-r">
        ...
        <ChromaticImage
          ...
          tilt={0.14}
          className="order-1 aspect-[4/5] min-h-96 bg-stone-300 lg:order-2 lg:aspect-auto lg:min-h-[42rem]"
        />
        <div className="order-3 border-t border-black/10 lg:border-t-0 lg:border-l">
