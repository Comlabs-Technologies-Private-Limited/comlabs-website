import Link from "next/link";
import { ArrowRight } from "lucide-react";

import { MarketingFadeIn } from "@/components/marketing/marketing-motion";
import type { CaseStudyServiceLink } from "@/lib/case-studies";
import { canonicalPath } from "@/lib/site";

type RelatedServiceProps = {
  services: CaseStudyServiceLink[];
};

export function RelatedService({ services }: RelatedServiceProps) {
  if (services.length === 0) return null;

  return (
    <MarketingFadeIn>
      <section className="border-t border-border px-6 py-12 md:py-16">
        <div className="mx-auto max-w-6xl">
          <div className="grid grid-cols-12 gap-8">
            <div className="col-span-12 lg:col-span-3">
              <p className="text-xs tracking-widest text-muted-foreground uppercase">
                {services.length > 1 ? "Related services" : "Related service"}
              </p>
            </div>
            <div className="col-span-12 flex flex-col gap-6 lg:col-span-7">
              {services.map((service) => (
                <Link
                  key={service.href}
                  href={canonicalPath(service.href)}
                  className="group inline-flex flex-col gap-1 transition-colors"
                >
                  <span className="text-base font-medium tracking-tight text-foreground group-hover:text-[var(--warm-orange)]">
                    {service.label}
                  </span>
                  <span className="inline-flex items-center gap-1.5 text-sm text-muted-foreground group-hover:text-foreground">
                    {service.description}
                    <ArrowRight size={14} aria-hidden="true" />
                  </span>
                </Link>
              ))}
            </div>
          </div>
        </div>
      </section>
    </MarketingFadeIn>
  );
}
