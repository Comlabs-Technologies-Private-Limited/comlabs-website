  ...
  useEffect(() => {
    ...
    function getWebGLContext(target: HTMLCanvasElement) {
      ...
      let context: WebGLRenderingContext | null =
        (target.getContext("webgl2", params) as WebGL2RenderingContext | null) ?? null;
      const isWebGL2 = context instanceof WebGL2RenderingContext;
      if (!isWebGL2) {
        context =
          (target.getContext("webgl", params) as WebGLRenderingContext | null) ||
          (target.getContext("experimental-webgl", params) as WebGLRenderingContext | null);
      }
    ...
    function getSupportedFormat(
      ...
      if (!supportRenderTextureFormat(context, internalFormat, format, type)) {
        ...
        switch (internalFormat) {
          ...
          case gl2.RG16F:
            return getSupportedFormat(context, gl2.RGBA16F, context.RGBA, type);
          default:
            return null;
        }
      }
    ...
    function toDyeIntensity(color: RGB) {
      return { r: color.r * 0.15, g: color.g * 0.15, b: color.b * 0.15 };
    }

    function hexToRGB(hex: string) {
      return toDyeIntensity(parseHexRGB(hex));
    }
    ...
    function generateColor() {
      ...
      const primary = parseHexRGB(config.COLOR);
      const secondary = parseHexRGB(config.COLOR_SECONDARY);

      return toDyeIntensity({
        r: primary.r + (secondary.r - primary.r) * blend,
        g: primary.g + (secondary.g - primary.g) * blend,
        b: primary.b + (secondary.b - primary.b) * blend,
      });
    }
    ...
    function handleMouseDown(e: MouseEvent) {
      ...
      const { posX, posY } = getPointerPosition(e.clientX, e.clientY);
    ...
    function handleTouchStart(e: TouchEvent) {
      ...
      for (let i = 0; i < touches.length; i++) {
        ...
        const { posX, posY } = getPointerPosition(touches[i].clientX, touches[i].clientY);
